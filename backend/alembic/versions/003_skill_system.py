...
def upgrade():
    ...
    op.create_table(
        ...
        # FIX: Added agentium_id column that ORM expects
    ...
    op.create_index('ix_skills_agentium_id', 'skills', ['agentium_id'], unique=True)  # Added index
    ...
    op.create_table(
        'skill_submissions',
        sa.Column('id', sa.Integer(), autoincrement=True, nullable=False),
        # FIX: Added agentium_id column for consistency
        sa.Column('agentium_id', sa.String(length=20), unique=True, nullable=True),
        sa.Column('created_at', sa.DateTime(), nullable=True),
        sa.Column('updated_at', sa.DateTime(), nullable=True),
